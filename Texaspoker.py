...
class bet:
    ...
    def __init__(self, comp_score, human_score):
        ...
        if not type(self.human_score) is int:            # Check the datatype of human_score is correct
            raise TypeError('Your bet input need to be an integer')
...
def continue_game(pair):
    ...
    try:                                # Check if the arguments are intactlly passed to the funciton
        len(pair) == 3
    except:
...
def run():
    ...
    while True:
        ...
        if command == 'final':
            ...
            print('Warning! Please enter a valid command in the provided chocie')
